...
const audit={schemaVersion:1,createdAt:new Date().toISOString(),candidateInput:candidatePath,database:candidateReport.database,tenant:candidateReport.tenant,snapshotId:evidence.snapshotId,reviewEvidenceSummary:{verified:Object.values(evidence.recipes).filter((entry)=>entry.status==='verified').length,unverified:Object.values(evidence.recipes).filter((entry)=>entry.status!=='verified').length},target:selection.target,selected:selection.selected.map((candidate)=>({sourceRecipeId:candidate.sourceRecipeId,title:candidate.title,locations:candidate._locationIds})),exclusionCounts:selection.exclusionCounts,coverage:selection.coverage,decision:'review-evidence-gate'};
