...
WITH target AS (
  ...
  SELECT id,'reviewEvidenceStatus'::text AS key,'string'::property_value_type AS value_type,status::text AS value,NULL::bigint AS value_integer,NULL::timestamptz AS value_datetime FROM target
  UNION ALL SELECT id,'reviewEvidenceCheckedAt','datetime',NULL,NULL,"checkedAt" FROM target
  UNION ALL SELECT id,'reviewEvidenceHtmlSha256','string',COALESCE("htmlSha256",''),NULL,NULL FROM target
