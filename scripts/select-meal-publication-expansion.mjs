...
function classifyDishKind(title) {
  if(/요거트|요구르트|토스트|샌드위치|스콘|머핀|케이크|카스테라|크로플|디저트|간식|주스|스무디|쉐이크|라떼|와플|팬케이크|핫케익|카나페|까나페|파르페|그래놀라|뮤즐리|청\s*(?:만들기)?|잼\s*(?:만들기)?/.test(title)) return 'breakfast';
  if(/달걀찜|계란찜|달걀말이|계란말이/.test(title)) return 'side';
  if(/볶음밥|비빔밥|덮밥|오므라이스|국밥|카레|파스타|스파게티|국수|수제비|라면|우동|소바|냉면|전골|찌개|감자탕|육개장|닭개장|냉국|(?:국|탕|수프|스프|죽)(?:\s|$|[),])/u.test(title)) return 'main';
