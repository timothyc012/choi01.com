...
test('all six entry pages are identical and use current recipes without portion costs', () => {
  ...
  for (const path of paths) {
    ...
    for (const asset of ['ontology-recipe-details', 'ontology-recipe-popular', 'meal-planner-recipe-data', 'meal-data-loader', 'meal-shopping']) {
      assert.ok(html.includes(asset + '.js?v='));
    }
    assert.doesNotMatch(html,/meal-package-prices\.js/);
