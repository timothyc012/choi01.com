...
export function CalendarView() {
  ...
  const weekdayLong = new Date(selected).toLocaleDateString('de-DE', { weekday: 'long' });
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <div className="kicker blue">Kalender · 일정</div>
          <h1>일정 관리</h1>
          <div className="sub">모임·세무 신고 마감일·헌금 일정 관리</div>
        </div>
        <div className="cal-head">
          <button className="btn sm" onClick={() => move(-1)} aria-label="이전 달">
            ←
          </button>
          <b className="mono" style={{ minWidth: 118, textAlign: 'center' }}>
            {monthTitleKR(cursor.y, cursor.m)}
          </b>
          <button className="btn sm" onClick={() => move(1)} aria-label="다음 달">
            →
          </button>
        </div>
      </div>
      ...
      <div className="grid-2" style={{ gridTemplateColumns: 'minmax(0, 1.7fr) minmax(280px, 1fr)' }}>
        ...
        <section className="panel">
          <div className="panel-head">
            <h3>
              {fmtDE(selected)} <span className="faint mono tiny">· {weekdayLong}</span>
            </h3>
              ...
              {dayEvents.map((ev) => (
                <div key={ev.id} className="memo-card">
                  <div className="row" style={{ justifyContent: 'space-between' }}>
                    <b style={{ fontSize: 13 }}>
                      {ev.time && <span className="mono faint">{ev.time} · </span>}
                      {ev.title}
                    </b>
