...
import { buildGnucashCsv } from '../lib/gnucash';
...
export function DatevFibu() {
  ...
  function exportGnucash() {
    const cash = data.cashbook.filter((e) => e.date.startsWith(month)).map(cashToBooking);
    const book = data.journal.filter((e) => e.date.startsWith(month)).map(journalToBooking);
    downloadFile(
      `gnucash-buchungen-${month}.csv`,
      buildGnucashCsv([...cash, ...book]),
      'text/csv;charset=utf-8',
    );
  }
      ...
      <section className="panel">
        <div className="panel-head">
          ...
          <div className="row">
            ...
            <button className="btn amber sm" onClick={exportGnucash}>
              GnuCash CSV (출납부+기장)
            </button>
          </div>
        </div>
